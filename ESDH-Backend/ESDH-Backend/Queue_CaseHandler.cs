...
namespace ESDH_Backend {
    public class Queue_CaseHandler {
        private readonly ILogger<Queue_CaseHandler> _logger;
        private readonly IConfiguration _config;
        private readonly string UrlPrefix = "ESDHCase_";

        public Queue_CaseHandler(ILogger<Queue_CaseHandler> logger, IConfiguration config) {
            _logger = logger;
            _config = config;
        }

        [Function(nameof(Queue_CaseHandler))]
        public void Run([QueueTrigger("esdhcase-queue")] QueueMessage message) {
            ...
            List spList = esdhCtx.Web.Lists.GetById(config.CaseListGuid);
            esdhCtx.Load(spList);
            esdhCtx.ExecuteQuery();
        ...
        private void CreateCustomerSite(AuthenticationManager auth, ClientContext esdhCtx, ListItem item, FunctionConfiguration config) {
            ...
            SharePoint.SetSiteProperties(ctx, config.EsdhSiteUrl, config.CaseListGuid, item.Id, config.TemplateLibraryGuid, "Case");
            ...
            _logger.LogInformation($"Setting 'Documents' library");
            List documentsList = SetDocumentsLibrary(ctx, config);
            ...
            SetNavigation(ctx, esdhCtx.Web.Url, documentsList.RootFolder.ServerRelativeUrl);

            _logger.LogInformation($"Setting homepage");
            SetHomepage(ctx, item, documentsList);
        ...
        private static List SetDocumentsLibrary(ClientContext ctx, FunctionConfiguration config) {
            List documentsList = ctx.Web.Lists.GetByTitle("Documents");
            ctx.Load(documentsList, p => p.RootFolder.ServerRelativeUrl, p => p.Id, p => p.Title, p => p.Fields, p => p.RoleAssignments, p => p.HasUniqueRoleAssignments);
            ...
            if (documentsList.HasUniqueRoleAssignments) {
                documentsList.ResetRoleInheritance();
                ctx.ExecuteQuery();
            }

            documentsList.BreakRoleInheritance(false, true);
            ...
            documentsList.RoleAssignments.Add(ctx.Web.EnsureUser(config.AdminGroup), editRDBC);
            documentsList.RoleAssignments.Add(ctx.Web.EnsureUser(config.MemberGroup), fullControlRDBC);
        ...
        private static void SetNavigation(ClientContext ctx, string esdhHomeUrl, string documentListUrl) {
        ...
        private static void SetHomepage(ClientContext ctx, ListItem item, List documentsList) {
            ...
            //        bingMapProps = Helpers.Constants.bingMapJson;
            ...
            homepage.AddControl(detailsWebpart, homepage.Sections[0].Columns[1], 1);
            ...
            //homepage.AddControl(bingMapWebpart, homepage.Sections[0].Columns[1], 2);
            ...
            //notesWebpart.PropertiesJson = Helpers.Constants.listProps.Replace("#listGuid#", notesList.Id.ToString()).Replace("#isLibrary#", "false").Replace("#listTitle#", notesList.Title);
