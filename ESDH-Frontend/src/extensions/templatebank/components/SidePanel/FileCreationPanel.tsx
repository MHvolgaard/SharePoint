import { ActionButton, DefaultButton, Label, PrimaryButton, Spinner, SpinnerSize, TextField } from '@fluentui/react';
import * as React from 'react';
import { SPService } from '../../services/SPService';
import { checkFilenameRequirements, formatLongDateTime, getFileType, getFileUriScheme } from '../../helpers/Helpers';
...
export const FileCreationPanel: React.FunctionComponent<IFileCreationPanelProps> = (props: React.PropsWithChildren<IFileCreationPanelProps>) => {
	...
	async function createDocument(open: boolean): Promise<void> {
		...
		const singleTemplate: TTemplatePayload = {
			...
			SourceFiles: [
				{
					...
					FileUrl: location.origin + props.selectedItem.FileRef
				}
			]
		};
		...
		try {
			response = await AFService.generateFileFromTemplates(singleTemplate)
		} catch (error) {
						...
						<div className={styles.fileInfo}>
							<div className={styles.fileInfoRow}>
								...
								<div className={styles.fileRow}>
									{fileIcon(props.selectedItem.Name, false)}
